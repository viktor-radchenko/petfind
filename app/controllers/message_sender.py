...
class MessageSender:
    ...
    def send_sms(self, message, recipient):
        ...
        link.save()
        body = f"Trace Return Alert ({data['tag_id']}). Visit https://{current_app.config['SERVER_NAME']}/dashboard for details. Check the location of the sender: {url_for('main.redirect_to_url', short_url=link.short_url, _external=True)}"  # noqa 501
